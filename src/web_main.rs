    ...
    impl Em100WebApp {
        ...
        fn set_chip(&mut self, chip: Rc<ChipDesc>) {
            ...
            spawn_local(async move {
                ...
                let (result, device) = if let Some(mut dev) = device {
                    // Stop emulation before changing chip type (set_chip_type
                    // does not stop, matching em100).
                ...
                match result {
                    Some(Ok(_)) => {
                        // Emulation was stopped before the chip change.
                        s.is_running = false;
                    ...
                    Some(Err(e)) => {
                        // Emulation was stopped before the failure.
                        s.is_running = false;
