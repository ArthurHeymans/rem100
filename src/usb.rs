    ...
    Dir: EndpointDirection,
{
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        ...
        let completion: Result<Completion> =
            race(async { Ok(endpoint.next_complete().await) }, async {
        ...
        match completion {
            ...
            Err(Error::Timeout) => {
                ...
                // completions until the endpoint is clean, exactly like
                // nusb's transfer_blocking does. The drain is deliberately
                // unbounded: a stale Cancelled completion consumed as a
                // later operation's result would silently cross operation
                // buffers, which is worse than waiting here on the error
                // path. Submits are sequential, so at most one completion
                // can be outstanding.
                endpoint.cancel_all();
                while endpoint.pending() > 0 {
                    // next_complete is cancellation-safe; just wait for
                    // the cancelled completion to come back.
...
///
/// Threads cannot block on wasm32, so this uses a JS timeout there.
/// Native callers drive the future to completion with
